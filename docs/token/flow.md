...
flowchart TD
    ...
    %% 認可コード取得〜トークン払い出し
    GET_AUTHORIZATION_CODE_INFO-->
    CHECK_AUTHORIZATION_CODE_EXISTS


    %% 認可コード存在チェック
    CHECK_AUTHORIZATION_CODE_EXISTS-->
    |認可コードがない場合| RETURN_AUTHORIZATION_CODE_NOT_EXISTS-->
    RECEIVE_USECASE_RESULT

    CHECK_AUTHORIZATION_CODE_EXISTS-->
    |認可コードがある場合| RECONCILE_PARAMETERS
    ...
    subgraph USECASE_AREA["ユースケース層"]
        ...
        CHECK_AUTHORIZATION_CODE_EXISTS{認可コード存在チェック}
        RETURN_AUTHORIZATION_CODE_NOT_EXISTS[認可コード存在チェックエラー返却]
        RECONCILE_PARAMETERS{
		client_idの突合
	    リダイレクトURLの突合
		認可コード期限のチェック
        }
        RETURN_PARAMETER_CHECK_ERROR[パラメータ検証エラー返却]
