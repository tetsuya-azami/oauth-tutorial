...
flowchart TD
    ...
    AUTH_CODE_REPOSITORY --> AUTH_CODE_SAVED_CHECK{認可コード保存チェック}

    AUTH_CODE_SAVED_CHECK -->|失敗| RETURN_STORAGE_ERROR[ストレージエラー返却]
    RETURN_STORAGE_ERROR --> RECEIVE_USECASE_RESULT
    AUTH_CODE_SAVED_CHECK -->|成功| RECEIVE_USECASE_RESULT
