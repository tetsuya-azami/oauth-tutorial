...
flowchart TD
    ...
    CHECK_IS_APPROVED -->|承認| AUTHENTICATE_USER{ユーザ認証}
    AUTHENTICATE_USER-->|認証OK| PUBLISH_AUTHORIZATION_CODE
    AUTHENTICATE_USER-->|認証NG| USER_AUTHENTICATION_ERROR
    USER_AUTHENTICATION_ERROR[ユーザ認証失敗エラー返却] --> RECEIVE_USECASE_RESULT
    PUBLISH_AUTHORIZATION_CODE[認可コード発行]
    PUBLISH_AUTHORIZATION_CODE --> SAVE_AUTH_CODE_TO_STORAGE[認可コードをストレージに保存]
    SAVE_AUTH_CODE_TO_STORAGE --> REMOVE_AUTH_PARAM[認可コードリポジトリ]
    REMOVE_AUTH_PARAM[認可リクエスト情報削除] --> REMOVE_AUTH_PARAM_FROM_SESSION[セッションから認可リクエスト情報を削除]
    REMOVE_AUTH_PARAM_FROM_SESSION --> SEND_RESULT_TO_HANDLER[ハンドラーに結果を返却]
    ...
    HANDLE_ERROR_RESPONSE --> ERROR_RESPONSE[エラーレスポンス]
    ...
    subgraph DB_AREA["データ層"]
        ...
        REMOVE_AUTH_PARAM_FROM_SESSION
    end
